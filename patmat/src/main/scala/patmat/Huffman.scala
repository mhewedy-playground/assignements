...
object Huffman {
   ...
   */
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = ??? /* Optional, will not implement */
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      _quickEncode(convert(tree))(text)
  }
  
  def _quickEncode(codeTable: CodeTable)(text: List[Char]): List[Bit] = text match{
      case List() => List()
      case x :: xs => codeBits(codeTable)(x) ::: _quickEncode(codeTable)(xs)
  }
}
